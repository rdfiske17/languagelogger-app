...
import de.lmu.ifi.researchime.data.keyboard.model.KeyboardLayoutModel;
import de.lmu.ifi.researchime.data.keyboard.model.KeyboardModel;
import de.lmu.ifi.researchime.data.keyboard.model.KeyboardRowModel;
...
public class KeyboardBuilder<KP extends KeyboardParams> {
    ...
    public KeyboardBuilder<KP> load(final int xmlId, final KeyboardId id) {
        ...
        KeyboardLayoutModel layoutModel = null;
        StateInteractor<KeyboardContainer> interactor = KeyboardInteractorRegistry.getKeyboardInteractor(mContext);
        KeyboardModel model = interactor.getModel().getEnabledKeyboard();
        if(model != null){
            if(id.isAlphabetKeyboard()){
                layoutModel = model.getAlphabetLayout(id.getLocale());
            }else if(id.isSymbolsKeyboard()){
                layoutModel = model.getSymbolsLayout();
            }else if(id.isSymbolsShiftedKeyboard()){
                layoutModel = model.getSymbolsShiftedLayout();
            }
            interactor.getModel().onKeyboardActive(model.getKeyboardId());
        }

        try {
            parseKeyboard(parser, layoutModel);
    ...
    private void parseKeyboard(final XmlPullParser parser, final KeyboardLayoutModel layout)
        ...
        while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
            ...
            if (event == XmlPullParser.START_TAG) {
                ...
                if (TAG_KEYBOARD.equals(tag)) {
                    ...
                    startKeyboard();
                    parseKeyboardContent(parser, layout, false);
                    return;
                }
    ...
    private void parseKeyboardContent(final XmlPullParser parser, final boolean skip)
            throws XmlPullParserException, IOException {
        parseKeyboardContent(parser, null, skip);
    }

    private void parseKeyboardContent(final XmlPullParser parser, final KeyboardLayoutModel layout, final boolean skip)
            throws XmlPullParserException, IOException {
        while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
            final int event = parser.next();
            if (event == XmlPullParser.START_TAG) {
                ...
                if (TAG_ROW.equals(tag)) {
                    if(layout != null && layout.hasRows()){
                        parseKeyboardModel(layout, skip);
                        endKeyboard();
                        return;
                    }
    ...
    private void parseKeyboardModel(KeyboardLayoutModel layout, final boolean skip) throws XmlPullParserException, IOException {
        for(KeyboardRowModel rowModel : layout.getRows()){
            final KeyboardRow row = parseRowAttributes(rowModel);
            if(! skip){
                startRow(row);
            }
            parseRowContent(rowModel, row, skip);
        }
    }

    private KeyboardRow parseRowAttributes(final KeyboardRowModel rowModel){
        return new KeyboardRow(mParams, mCurrentY, rowModel.getKeyWidth(), rowModel.getKeyHeight());
    }
    ...
    private void parseRowContent(final KeyboardRowModel rowModel, final KeyboardRow row, final boolean skip) throws XmlPullParserException, IOException {
        if(! rowModel.hasKeys()){
            return;
        }

        for(KeyboardKeyModel keyModel : rowModel.getKeys()){
            endKey(new Key(keyModel, mParams, row));
        }
        if(! skip){
            endRow(row);
        }
    ...
    private void endKeyboard() {
        ...
        if(mParams.mId.isCustomizableKeyboard()){
            //ResearchIME - always set occupiedHeight to actualHeight to be able to use smaller rows
            mParams.mOccupiedHeight = actualHeight;
        }else{
