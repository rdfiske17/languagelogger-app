...
public final class KeyboardRow {
    ...
    private static class RowAttributes {
         ...
         */
        public RowAttributes(final float percentage, final int keyboardWidth){
            mDefaultKeyWidth = percentage * keyboardWidth;
     ...
     * Custom constructor for dynamic keyboard layout
     */
    public KeyboardRow(final KeyboardParams params, final int y, final float keyWidth, final float keyHeight){
        mParams = params;
        mCurrentX = 0.0f;
        mCurrentY = y;
        mRowHeight = (int) ((keyHeight > 0 ? keyHeight : 1) * params.mDefaultRowHeight);
        mRowAttributesStack.push(new RowAttributes(keyWidth, params.mBaseWidth));
    }
    ...
    public float getKeyX(final float keyXPos){
        if(keyXPos == 0){
            return mCurrentX;
        }
        final float absKeyXPos = keyXPos * mParams.mBaseWidth;
        if(absKeyXPos > 0){
            return absKeyXPos;
        }
        ...
        return Math.max(absKeyXPos + keyboardRightEdge, mCurrentX);
    }
    ...
    public float getKeyWidth(final boolean widthFillRight, final float keyXPos, final float widthPercentage){
        if(widthFillRight){
        ...
        if(widthPercentage != 0){
            return widthPercentage * mParams.mBaseWidth;
        }
