...
import de.lmu.ifi.researchime.data.keyboard.model.KeyboardKeyModel;
...
public class Key implements Comparable<Key> {
     ...
     * Custom key
     */
    private KeyboardKeyModel key;

    /**
     * ResearchIME
     * Custom constructor for dynamic keyboard layout
     */
    public Key(@Nonnull KeyboardKeyModel key, @Nonnull final KeyboardParams params, @Nonnull final KeyboardRow row) {
        this.key = key;
        ...
        final float keyXPos = row.getKeyX(key.getXPos());
        final float keyWidth = row.getKeyWidth(key.isWidthFillRight(), keyXPos, key.getWidthPercentage());
        ...
        mHitBox.set(Math.round(keyXPos), keyYPos, Math.round(keyXPos + keyWidth) + 1, keyYPos + rowHeight);
        // Update row to have current x coordinate.
        row.setXPos(keyXPos + keyWidth);

        final String layoutSetCondition = KeyboardId.elementIdToName(params.mId.mElementId);
        final String modeCondition = KeyboardId.modeName(params.mId.mMode);
        final int imeActionCondition = params.mId.imeAction();
        mBackgroundType = backgroundType(key.getBackgroundType(layoutSetCondition, modeCondition, imeActionCondition));

        final int baseWidth = params.mBaseWidth;
        final int visualInsetsLeft = Math.round(key.getVisualInsetsLeft() * baseWidth);
        final int visualInsetsRight = Math.round(key.getVisualInsetsRight() * baseWidth);
        if(key.getKeyLabelFlags() != 0){
            mLabelFlags = key.getKeyLabelFlags();
        }else{
            mLabelFlags = row.getDefaultKeyLabelFlags();
        }

        boolean needsToUpcase = needsToUpcase(mLabelFlags, params.mId.mElementId);
        final Locale localeForUpcasing = params.mId.getLocale();
        int actionFlags = key.getActionFlags();

        String[] moreKeys = key.getMoreKeys();
        // Get maximum column order number and set a relevant mode value.
        int moreKeysColumnAndFlags = params.mMaxMoreKeysKeyboardColumn;
        int value;
        ...
        if ((mLabelFlags & LABEL_FLAGS_DISABLE_ADDITIONAL_MORE_KEYS) != 0) {
            ...
            additionalMoreKeys = key.getAdditionalMoreKeys();
        }
        ...
        final String keySpec = key.getKeySpec(layoutSetCondition, modeCondition, imeActionCondition);
        ...
        final int altCodeInAttr =  CODE_UNSPECIFIED;
        final int altCode = needsToUpcase
                ? StringUtils.toTitleCaseOfKeyCode(altCodeInAttr, localeForUpcasing)
                : altCodeInAttr;

        final int disabledIconId = KeySpecParser.getIconId(key.getIconDisabled());
    ...
    private static int backgroundType(final String backgroundName) {
        if(backgroundName != null){
            switch (backgroundName) {
                case "empty": return BACKGROUND_TYPE_EMPTY;
                case "normal": return BACKGROUND_TYPE_NORMAL;
                case "functional": return BACKGROUND_TYPE_FUNCTIONAL;
                case "stickyOff": return BACKGROUND_TYPE_STICKY_OFF;
                case "stickyOn": return BACKGROUND_TYPE_STICKY_ON;
                case "action": return BACKGROUND_TYPE_ACTION;
                case "spacebar": return BACKGROUND_TYPE_SPACEBAR;
            }
        }
        return BACKGROUND_TYPE_NORMAL;
    }
    ...
    public final int selectTextSize(final KeyDrawParams params) {
        if(key != null && key.hasTextSize()){
            return key.getTextSize();
        }
    ...
    public final int selectHintTextSize(final KeyDrawParams params) {
        if(key != null && key.hasHintTextSize()){
            return key.getHintTextSize();
        }
